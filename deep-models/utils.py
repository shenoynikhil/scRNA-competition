...
def train_all_epochs(model, config, save_models, fold, training_loader, validation_loader):
    epoch_number = 0
    ...
    for epoch in range(config['model_params']['epochs']):
        ...
        if save_models and  avg_vloss < best_vloss:
            ...
            model_path = os.path.join(config['output_dir'], './models/{}_fold{}_epoch{}'.format(model.name, fold, epoch_number))
...
def train(model, config, save_models, fold, x_train, y_train, x_val, y_val):
    ...
    train_all_epochs(model, config, save_models, fold, training_loader, validation_loader)
