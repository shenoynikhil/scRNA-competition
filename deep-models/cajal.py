...
class CajalWrapper:
    ...
    def preprocess(self, x_train: ad.AnnData, x_test: ad.AnnData, y: ad.AnnData):
        ...
        for batch in train_batches:
            x_train.obs["batch_median"][x_train.obs.donor == batch] = np.median(train_total[x_train.obs.donor == batch])
            x_train.obs["batch_sd"][x_train.obs.donor == batch] = np.std(train_total[x_train.obs.donor == batch])
        ...
        for batch in test_batches:
            x_test.obs["batch_median"][x_test.obs.donor == batch] = np.median(test_total[x_test.obs.donor == batch])
            x_test.obs["batch_sd"][x_test.obs.donor == batch] = np.std(test_total[x_test.obs.donor == batch])
    ...
    def initialize(self) -> Cajal:
        ...
        return Cajal(hp, self.input_shape, self.output_shape, self.min_val, self.max_val)
